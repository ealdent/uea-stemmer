...
You can also extract the stemmed word along with the rule by using the `decorated_stem` method.

  stem = stemmer.decorated_stem('invited')   # Word('invite', Rule #22)
  puts stem.rule  # rule #22 (remove -ed when the word ends in -ted)
